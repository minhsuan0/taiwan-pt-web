...
export function formatForPrompt(rerankedResults) {
  ...
  const lines = ['=== 精選實證段落（已依相關性與實證等級排序，請務必引用並附上實證微膠囊）==='];

  rerankedResults.forEach(({ chunk, finalScore, ceScore, evidenceMultiplier }, idx) => {
    const iv = chunk.intervention || {};
    const yearMatch = (chunk.source || '').match(/\b(20\d{2}|19\d{2})\b/);
    const year = yearMatch ? yearMatch[1] : '2023';
    const chineseTitle = (iv.name || chunk.topic || '物理治療臨床實證指引').replace(/[\|\(\)]/g, ' ').trim();
    const cleanOutcome = (chunk.outcome || '經臨床研究證實能顯著減輕疼痛並改善日常活動功能').replace(/[\|\(\)]/g, ' ').trim();
    const shortSource = (chunk.source || 'JOSPT / 臨床實證指引').replace(/[\|\(\)]/g, ' ').trim();
    const url = chunk.url || (chunk.pmid ? `https://pubmed.ncbi.nlm.nih.gov/${chunk.pmid}/` : '');
    const tag = `[Level ${chunk.evidence_level}](evidence:${chunk.evidence_level}|${year}|${chineseTitle}|${cleanOutcome}|${shortSource}|${url})`;
...
【段落 ${idx + 1}】主題：${chunk.topic}（分期：${chunk.phase}）
 ...
 臨床效益：${chunk.outcome || '—'}
 實證等級：Level ${chunk.evidence_level}（相關性分數：${finalScore.toFixed(3)}）
 來源：${chunk.source}（PMID: ${chunk.pmid || 'N/A'}）
 連結：${url || '—'}
 🎯 實證微膠囊標籤（請直接複製貼在回答動作後方）：${tag}`);
  });

  lines.push('\n（強制要求：在回答【你可以這樣做（建議運動與日常調整）】的各項動作或效益後，務必附上上述 🎯 實證微膠囊標籤，讓前端能顯示 Level 徽章並可點擊查看實證量尺！）');
  return lines.join('\n');
}
